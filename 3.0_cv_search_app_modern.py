        ...
        try:
            ...
            for slide in prs.slides:
                for shape in slide.shapes:
                    if shape.has_text_frame and "{{RIASSUNTO}}" in shape.text:
                        self.replace_text_with_font_size(shape, "{{RIASSUNTO}}", data.get("summary", ""), 9)
            ...
            for slide in prs.slides:
                for shape in slide.shapes:
                    ...
                    if "{{COMPETENZA}}" in text:
                        self.fill_list(shape, data.get("skills", []))
                    elif "{{TECNOLOGIE}}" in text:
                        self.fill_list(shape, data.get("technologies", []))
...
class PPTXGeneratorGeneric:
    """Generatore PPTX generico per template con placeholder standard come 00_Template_CV"""
    def __init__(self, template_path, logger=None):
        self.template_path = Path(template_path)
        self.logger = logger or Logger()
    ...
    def generate_cv(self, json_path, output_path):
        ...
        try:
            ...
            # Mapping per 00_Template_CV.pptx
            campi = {
                "{{RUOLO}}": data.get("title", ""),
            ...
            for slide in prs.slides:
                for shape in slide.shapes:
                    ...
                    if "{{ESPERIENZE}}" in text:
                        experiences_text = []
                        for exp in data.get("experience", []):
                            exp_line = f"• {exp.get('company', '')} ({exp.get('period', '')}): {exp.get('description', '')}"
                    ...
                    # SKILLS
                        ...
                        self.fill_list(shape, skills)
                        self.logger.log(f"Compilato SKILLS: {len(skills)} items")
                    
                    # LINGUE - Se presente nel JSON
                    elif "{{LINGUE}}" in text:
                        # Assumendo che lingue sia un campo custom o estrai da certifications
                        lingue = data.get("languages", ["Italiano (madrelingua)", "Inglese (fluente)"])
...
def create_generator_for_template(template_path, logger):
    """Factory per creare il generator corretto in base al template"""
    template_name = Path(template_path).stem.lower()
    
    if "acn_1" in template_name or "template_acn_1" in template_name:
        return PPTXGeneratorACN1(template_path, logger)
    else:
        # Usa il generatore generico per tutti gli altri template
...
class CVSearchApp:
    ...
    def setup_ui(self):
        ...
        # Query Section
        query_header = ctk.CTkFrame(left_column, fg_color=("gray85", "gray17"), corner_radius=8)
        query_header.pack(fill="x", padx=10, pady=(10, 5))
        
        query_label = ctk.CTkLabel(query_header, text="📝 Descrizione della Gara",
                                font=ctk.CTkFont(size=14, weight="bold"))
        ...
        self.query_text.insert("1.0", "Nome Gara: ...\ncompetenze obbligatorie: ...")

        # Control Panel
        control_header = ctk.CTkFrame(left_column, fg_color=("gray85", "gray17"), corner_radius=8)
        control_header.pack(fill="x", padx=10, pady=(0, 5))
        
        control_label = ctk.CTkLabel(control_header, text="⚙️ Impostazioni Ricerca",
                                    font=ctk.CTkFont(size=14, weight="bold"))
        ...
        # Search Button
        self.search_button = ctk.CTkButton(control_content, 
        ...
        # Results Textbox - Occupa tutto lo spazio
    ...
    def load_data(self):
        ...
        try:
            ...
            if missing:
                messagebox.showerror("Errore", f"File mancanti: {', '.join(missing)}\n\nEsegui prima create_embeddings.py")
                self.status_label.configure(text="❌ File mancanti!", text_color="red")
                self.logger.log(f"File mancanti: {missing}", "ERROR")
                return
            
            self.status_label.configure(text="⏳ Caricamento modello...", text_color="orange")
            self.root.update()
            ...
            self.status_label.configure(text=f"✅ Sistema pronto! {len(self.cv_labels)} CV caricati", 
                                       text_color="#2CC985")
